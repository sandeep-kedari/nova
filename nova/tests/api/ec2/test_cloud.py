...
from nova.image import glance as service
...
class CloudTestCase(test.TestCase):
	...
	metadata = self.cloud.compute_api.get_instance_metadata(self.context,
        ...
        #Testing Create_tag for image[sandeep]
	...
	#if 'metadata' not in image_meta1['properties']:
	  #  meta = None 
	#else:
	#    meta = image_meta1['properties']['metadata']	 
	...
	# Resource ID for image
        ...
        tags = self.cloud.describe_tags(self.context,
                filter=[{'name': 'value',
                         'value': ['quu?']}])['tagSet']
        self.assertEqualSorted(tags, [image2_key_baz, inst2_key_baz])
